...
import { updateOfferServer } from "../actions/server/updateOfferServer";
...
import { OfferGetPayload } from "@/generated/prisma/models";

type OfferWithLandingPages = OfferGetPayload<{
  include: { landingPages: { select: { id: true } } };
}>;

export default function EditOfferForm({ offer }: { offer: OfferWithLandingPages }) {
  ...
  const form = useForm({
    defaultValues: {
      ...
      landingPageIds: offer.landingPages.map((lp) => lp.id),
