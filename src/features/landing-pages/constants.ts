...
const messageBodyAdmin = (customerName: string, orderId: OrderModel["id"], totalPrice: OrderModel["totalPrice"]) =>
  `New order received! Order #${orderId} has been placed by ${customerName}. Total Price: ৳${totalPrice}.`;
...
export const SMSBangladeshUrl = (
  ...
  )}&to=88${encodeURIComponent(mobileNumber)}&text=${encodeURIComponent(isCustomer ? messageBody(customerName, orderId) : messageBodyAdmin(customerName, orderId, totalPrice!))}`;
